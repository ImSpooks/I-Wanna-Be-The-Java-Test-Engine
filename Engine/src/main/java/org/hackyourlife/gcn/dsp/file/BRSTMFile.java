...
public class BRSTMFile implements Stream {
	long	sample_count;
	long	nibble_count;
	long	sample_rate;
	int	loop_flag;
	...
	public BRSTMFile(RandomAccessFile file) throws FileFormatException, IOException {
		this.file = file;
	...
	public static Stream getFromFile(RandomAccessFile file) {
		try {
			try {
				return new BRSTMFile(file);
				...
				try {
					...
					e.printStackTrace();
					try {
						return new RS03File(file);
					} catch(FileFormatException exc) {
						e.printStackTrace();
						return new DSPFile(file);
					}
				}
			}
