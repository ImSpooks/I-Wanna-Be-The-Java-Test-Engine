package org.hackyourlife.gcn.dsp.stream;

import org.hackyourlife.gcn.dsp.ADPCMDecoder;
import org.hackyourlife.gcn.dsp.FileFormatException;
import org.hackyourlife.gcn.dsp.Stream;
import org.hackyourlife.gcn.dsp.endianess;

import java.io.BufferedInputStream;
import java.io.IOException;
import java.io.InputStream;

public class BRSTMStream implements Stream {
    private long sample_count;
    private long sample_rate;
    private int loop_flag;
    private long loop_start_sample;
    private long loop_end_sample;
    private long loop_start_offset;
    private long loop_end_offset;
    ...
    private long channel_start_offset[];
    private long channel_offset[];

    private long current_sample;
    private long current_byte;

    private long interleave_block_size;
    private long interleave_smallblock_size;
    ...
    public BRSTMStream(BufferedInputStream inputStream) throws FileFormatException, IOException {
        this.inputStream = inputStream;
        this.filesize = this.inputStream.available();
        this.inputStream.mark(Math.toIntExact(this.filesize));
        readHeader();
        reset();
    }

    public static int unsigned2signed16bit(int x) {
        int sign = x & (1 << 15);
        int value = x & ~(1 << 15);
    ...
    private void seek(long pos) throws IOException {
        inputStream.reset();
        inputStream.skip(start_offset + pos);
    ...
    private int[] doDecode() throws Exception {
        ...
        int read = inputStream.read(rawdata);
        filepos += read;
    ...
    public static Stream getFromStream(InputStream inputStream) {
        BufferedInputStream bufferedStream = inputStream instanceof BufferedInputStream ? (BufferedInputStream) inputStream : new BufferedInputStream(inputStream);

        try {
            try {
                return new BRSTMStream(bufferedStream);
                ...
                try {
                    ...
                    e.printStackTrace();
                    return new DSPStream(bufferedStream);
                }
            }
