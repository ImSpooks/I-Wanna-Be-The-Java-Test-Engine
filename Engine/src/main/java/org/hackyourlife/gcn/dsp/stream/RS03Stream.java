...
public class RS03Stream implements Stream {
	...
	private String path;
	public RS03Stream(String path) throws FileFormatException, IOException {
		this.inputStream = new BufferedInputStream(this.getClass().getResourceAsStream(this.path = path));
	...
	public boolean read_dsp_header(byte[] header, BufferedInputStream in) throws IOException {
	...
	public boolean open(String filename) throws Exception {
		if(inputStream != null)
			close();
		inputStream = new BufferedInputStream(this.getClass().getResourceAsStream(this.path));
	...
	private boolean readHeader() throws IOException {
		...
		if(!read_dsp_header(header, inputStream))
			return(false);
	...
	private void seek(long pos) throws IOException {
		inputStream.reset();
		inputStream.skip(startoffset + pos);
